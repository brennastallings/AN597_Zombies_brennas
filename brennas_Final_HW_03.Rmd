...
Also I was super dumb and calculated the variance for these but not the SD: I fixed it...
```{r}
#Height
mean.height<-mean(d$height)
height_v <- function(x){sum((x-mean(x))^2)/(length(x))} 
sd_height <- function(x){sqrt(height_v(x))}
sd.height <-sd_height(d$height)
mean.height
sd.height
...
sd_weight <- function(x){sqrt(weight_v(x))}
sd.weight <-sd_weight(d$weight)
mean.weight
sd.weight
...
sd_age <- function(x){sqrt(age_v(x))}
sd.age <-sd_age(d$age)
mean.age
sd.age
...
sd_killed <- function(x){sqrt(killed_v(x))}
sd.killed <-sd_killed(d$zombies_killed)
mean.killed
sd.killed
...
sd_school <- function(x){sqrt(school_v(x))}
sd.school <-sd_school(d$years_of_education)
mean.school
sd.school
```
...
# like how NIcola added different colors to her plot, so I tried it
h <- ggplot(data=d, aes(x=d$gender, y=d$height, color=gender))
h <- h + geom_boxplot()
h <- h + theme_bw()
h <- h + ylab("Height")
h <- h + xlab("Gender")
...
```{r}
w <- ggplot(data=d, aes(x=d$gender, y=d$weight, color=gender))
w <- w + geom_boxplot()
w <- w + theme_bw()
w <- w + ylab("Weight")
w <- w + xlab("Gender")
...
a <- ggplot(data=d, aes(x=d$gender, y=d$age, color=gender))
a <- a + geom_boxplot()
a <- a + theme_bw()
a <- a + ylab("Age")
a <- a + xlab("Gender")
...
KO <- ggplot(data=d, aes(x=d$gender, y=d$zombies_killed, color=gender))
KO <- KO + geom_boxplot()
KO <- KO + theme_bw()
...
y <- ggplot(data=d, aes(x=d$gender, y=d$years_of_education, color=gender))
y <- y + geom_boxplot()
y <- y + theme_bw()
...
```{r}
ggarrange(h, w, a, y, KO, nrows=2, common.legend = T, legend= "right")
#this makes it easier to look at all the boxplots together
```
...
# first, we build a plot object and arrange points by age
...
Both height and weight seem to be positivly related to age. As age increases, both other variables tend to increase as well. Age seems to have a higher correlation woth height than weight.
...
I think both of the other 2 are Poisson distributions
...
~I added some code to arrange the data a bit better
